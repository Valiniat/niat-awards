  ...
  <section className="py-14 sm:py-20 bg-background" id="categories">
    <div className="container">
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-10 sm:mb-14">
        ...
        <p className="text-muted-foreground max-w-2xl mx-auto text-base sm:text-lg">Four distinct categories celebrating different dimensions of exceptional teaching.</p>
      </motion.div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
        {categories.map((c, i) => (
          <motion.div key={c.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
            ...
            <p className="text-muted-foreground leading-relaxed text-sm sm:text-base">{c.desc}</p>
          </motion.div>
        ))}
      </div>
    </div>
